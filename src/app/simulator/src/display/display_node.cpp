...
Display::Display(const std::string& node_name, const rclcpp::NodeOptions& options)
    ...
    s_driveway1_ = this->create_subscription<ad_msgs::msg::PolyfitLaneData> (
        "driveway1", qos_profile, std::bind(&Display::CallbackDriveway1, this, std::placeholders::_1));
    s_driveway2_ = this->create_subscription<ad_msgs::msg::PolyfitLaneData> (
        "driveway2", qos_profile, std::bind(&Display::CallbackDriveway2, this, std::placeholders::_1));
    s_driveway3_ = this->create_subscription<ad_msgs::msg::PolyfitLaneData> (
        "driveway3", qos_profile, std::bind(&Display::CallbackDriveway3, this, std::placeholders::_1));
    ...
    p_driveway1_marker_ = this->create_publisher<visualization_msgs::msg::MarkerArray> (
        "driveway1_marker", qos_profile);
    p_driveway2_marker_ = this->create_publisher<visualization_msgs::msg::MarkerArray> (
        "driveway2_marker", qos_profile);
    p_driveway3_marker_ = this->create_publisher<visualization_msgs::msg::MarkerArray> (
        "driveway3_marker", qos_profile);
...
void Display::Run() {
    ...
    ad_msgs::msg::PolyfitLaneData driveway1; {
        if (b_is_driveway1_ == true) {
            std::lock_guard<std::mutex> lock(mutex_driveway1_);
            driveway1 = i_driveway1_;
        }
    }

    ad_msgs::msg::PolyfitLaneData driveway2; {
        if (b_is_driveway2_ == true) {
            std::lock_guard<std::mutex> lock(mutex_driveway2_);
            driveway2 = i_driveway2_;
        }
    }

    ad_msgs::msg::PolyfitLaneData driveway3; {
        if (b_is_driveway3_ == true) {
            std::lock_guard<std::mutex> lock(mutex_driveway3_);
            driveway3 = i_driveway3_;
        }
    }
    ...
    // Display all three driveways with different colors
    if (b_is_driveway1_ == true) {
        DisplayDrivewayWithColor(driveway1, current_time, 0.1, 30.0,
                                  1.0f, 0.0f, 0.0f, p_driveway1_marker_);  // Red
    }
    if (b_is_driveway2_ == true) {
        DisplayDrivewayWithColor(driveway2, current_time, 0.1, 30.0,
                                  0.0f, 1.0f, 0.0f, p_driveway2_marker_);  // Green
    }
    if (b_is_driveway3_ == true) {
        DisplayDrivewayWithColor(driveway3, current_time, 0.1, 30.0,
                                  0.0f, 0.0f, 1.0f, p_driveway3_marker_);  // Blue
    }
}
...
void Display::DisplayDrivewayWithColor(const ad_msgs::msg::PolyfitLaneData& driveway,
                                        const rclcpp::Time& current_time,
                                        const double& interval, const double& ROILength,
                                        float r, float g, float b,
                                        rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr& publisher) {

    double a0 = driveway.a0;
    double a1 = driveway.a1;
    double a2 = driveway.a2;
    double a3 = driveway.a3;
    ...
    while (distance_square < ROILength * ROILength) {
        ...
        visualization_msgs::msg::Marker marker;
        marker.header.frame_id = driveway.frame_id;
        marker.header.stamp = current_time;

        marker.ns = driveway.id;
        marker.id = id++;
