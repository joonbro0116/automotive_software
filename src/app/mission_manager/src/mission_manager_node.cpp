...
void ScenarioRunner::Run() {
    ...
    // Publish output
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - //
    p_detected_mission_->publish(ros2_bridge::UpdateMission(scc_objects, merge_objects, road_condition, road_slope));
    p_display_mission_->publish(ros2_bridge::UpdateMissionDisplay(display_scc_objects, display_merge_objects, display_road_slope, display_road_condition));
}
