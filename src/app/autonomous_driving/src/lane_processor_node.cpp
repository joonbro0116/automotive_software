...
// ROS2 includes for standalone node
...
class LaneProcessorNode : public rclcpp::Node {
public:
    LaneProcessorNode(const std::string& node_name, const rclcpp::NodeOptions& options = rclcpp::NodeOptions())
        : Node(node_name, options),
          b_is_vehicle_state_(false),
          b_is_lane_points_(false),
          b_is_manual_input_(false) {

        // QoS 설정
        auto qos_profile = rclcpp::QoS(rclcpp::KeepLast(10));

        // Parameters
        this->declare_parameter("lane_processor/loop_rate_hz", 100.0);
        this->get_parameter("lane_processor/loop_rate_hz", loop_rate_hz_);
        ...
        s_vehicle_state_ = this->create_subscription<ad_msgs::msg::VehicleState>(
            ...
            std::bind(&LaneProcessorNode::CallbackVehicleState, this, std::placeholders::_1));

        s_lane_points_ = this->create_subscription<ad_msgs::msg::LanePointData>(
            "lane_points", qos_profile,
            std::bind(&LaneProcessorNode::CallbackLanePoints, this, std::placeholders::_1));

        s_manual_input_ = this->create_subscription<ad_msgs::msg::VehicleCommand>(
            "/manual_input", qos_profile,
            std::bind(&LaneProcessorNode::CallbackManualInput, this, std::placeholders::_1));

        // Publishers
        p_lane_set_ = this->create_publisher<ad_msgs::msg::LanePointDataArray>(
            "lane_set", qos_profile);
        ...
        p_driveway1_ = this->create_publisher<ad_msgs::msg::PolyfitLaneData>(
            "driveway1", qos_profile);
        p_driveway2_ = this->create_publisher<ad_msgs::msg::PolyfitLaneData>(
            "driveway2", qos_profile);
        p_driveway3_ = this->create_publisher<ad_msgs::msg::PolyfitLaneData>(
            "driveway3", qos_profile);
        p_vehicle_command_ = this->create_publisher<ad_msgs::msg::VehicleCommand>(
            "vehicle_command", qos_profile);

        // Timer
        t_run_node_ = this->create_wall_timer(
            std::chrono::milliseconds(static_cast<int64_t>(1000 / loop_rate_hz_)),
            std::bind(&LaneProcessorNode::Run, this));
    }

    ~LaneProcessorNode() = default;
...
private:
    ...
    void CallbackLanePoints(const ad_msgs::msg::LanePointData::SharedPtr msg) {
        ...
        i_lane_points_.frame_id = msg->frame_id;
        i_lane_points_.id = msg->id;
        i_lane_points_.point.clear();

        for (const auto& p : msg->point) {
            interface::Point2D pt;
            pt.x = p.x;
            pt.y = p.y;
            i_lane_points_.point.push_back(pt);
        }
    ...
    void Run() {
        ...
        // vehicle_command는 lane_points 없이도 발행 (차량 움직임 우선)
        if (b_is_manual_input_) {
            std::lock_guard<std::mutex> lock(mutex_manual_input_);
            p_vehicle_command_->publish(i_manual_input_);
        }
        ...
        // Global points 가져오기
        const auto& lane1_global = lane_processor_.GetLane1PointsGlobal();
        const auto& lane2_global = lane_processor_.GetLane2PointsGlobal();
        const auto& lane3_global = lane_processor_.GetLane3PointsGlobal();
        const auto& lane4_global = lane_processor_.GetLane4PointsGlobal();

        // 메시지 생성
        ad_msgs::msg::LanePointDataArray lane_set_msg;
        lane_set_msg.frame_id = "world";

        // Lane 1
        ad_msgs::msg::LanePointData lane1_msg;
        lane1_msg.frame_id = "world";
        lane1_msg.id = "lane1";
        for (const auto& pt : lane1_global) {
            geometry_msgs::msg::Point p;
            p.x = pt.x;
            p.y = pt.y;
            p.z = 0.0;
            lane1_msg.point.push_back(p);
        }
        lane_set_msg.lane.push_back(lane1_msg);

        // Lane 2
        ad_msgs::msg::LanePointData lane2_msg;
        lane2_msg.frame_id = "world";
        lane2_msg.id = "lane2";
        for (const auto& pt : lane2_global) {
            geometry_msgs::msg::Point p;
            p.x = pt.x;
            p.y = pt.y;
            p.z = 0.0;
            lane2_msg.point.push_back(p);
        }
        lane_set_msg.lane.push_back(lane2_msg);

        // Lane 3
        ad_msgs::msg::LanePointData lane3_msg;
        lane3_msg.frame_id = "world";
        lane3_msg.id = "lane3";
        for (const auto& pt : lane3_global) {
            geometry_msgs::msg::Point p;
            p.x = pt.x;
            p.y = pt.y;
            p.z = 0.0;
            lane3_msg.point.push_back(p);
        }
        lane_set_msg.lane.push_back(lane3_msg);

        // Lane 4
        ad_msgs::msg::LanePointData lane4_msg;
        lane4_msg.frame_id = "world";
        lane4_msg.id = "lane4";
        for (const auto& pt : lane4_global) {
            geometry_msgs::msg::Point p;
            p.x = pt.x;
            p.y = pt.y;
            p.z = 0.0;
            lane4_msg.point.push_back(p);
        }
        lane_set_msg.lane.push_back(lane4_msg);

        // 발행: lane_set (global points)
        p_lane_set_->publish(lane_set_msg);

        // 발행: poly_lanes (polyfit 결과) - Process() 반환값 직접 사용
        // frame_id를 실제 TF 프레임 "ego/body"로 설정
        std::string frame_id = "ego/body";
        ...
        // 발행: driving_way (ego center lane)
        interface::PolyfitLane driving_way = lane_processor_.GetEgoLane();
        driving_way.frame_id = frame_id;
        p_driving_way_->publish(ros2_bridge::UpdatePolyfitLane(driving_way));

        // 발행: driveway1/2/3 (각 driveway 중심선)
        interface::PolyfitLane driveway1 = lane_processor_.GetDriveway1CenterLine();
        interface::PolyfitLane driveway2 = lane_processor_.GetDriveway2CenterLine();
        interface::PolyfitLane driveway3 = lane_processor_.GetDriveway3CenterLine();
        driveway1.frame_id = frame_id;
        driveway2.frame_id = frame_id;
        driveway3.frame_id = frame_id;
        driveway1.id = "driveway1";
        driveway2.id = "driveway2";
        driveway3.id = "driveway3";
        p_driveway1_->publish(ros2_bridge::UpdatePolyfitLane(driveway1));
        p_driveway2_->publish(ros2_bridge::UpdatePolyfitLane(driveway2));
        p_driveway3_->publish(ros2_bridge::UpdatePolyfitLane(driveway3));
    }
    ...
    // Publishers
    rclcpp::Publisher<ad_msgs::msg::LanePointDataArray>::SharedPtr p_lane_set_;
    ...
    rclcpp::Publisher<ad_msgs::msg::PolyfitLaneData>::SharedPtr p_driveway1_;
    rclcpp::Publisher<ad_msgs::msg::PolyfitLaneData>::SharedPtr p_driveway2_;
    rclcpp::Publisher<ad_msgs::msg::PolyfitLaneData>::SharedPtr p_driveway3_;
    ...
    bool b_is_lane_points_;
    bool b_is_manual_input_;

    // LaneProcessor instance
    lane_processor::LaneProcessor lane_processor_;
};

int main(int argc, char** argv) {
    std::string node_name = "lane_processor_node";

    rclcpp::init(argc, argv);
    rclcpp::spin(std::make_shared<LaneProcessorNode>(node_name));
    rclcpp::shutdown();
...
interface::PolyfitLanes LaneProcessor::ProcessNormalMode(
    ...
    // -------------------------------------------------------------------------
    // Step 1.5: 현재 vehicle frame 기준으로 폴리핏 갱신
    //           이전 프레임의 폴리핏은 이전 vehicle frame 기준이므로,
    //           ClassifyAndStoreLanesByPolyfit에서 사용하기 전에 갱신 필요
    // -------------------------------------------------------------------------
...
void LaneProcessor::FitLanePolynomials(
    ...
    // -------------------------------------------------------------------------
    // Step 3: 이번 프레임 "실측" 기준으로 primary 2개 선택 (x_max 큰 순)
    // -------------------------------------------------------------------------
    DeterminePrimaryLanesFromRealPoints();

    // -------------------------------------------------------------------------
    // Step 4: Primary 레인만 피팅 (non-primary는 외분/내분으로 생성되므로 불필요)
    // -------------------------------------------------------------------------
...
void LaneProcessor::FilterStoredPointsByRange(
    ...
    static constexpr double MAX_LATERAL_MEMORY = 12.0;
...
const std::vector<interface::Point2D>& LaneProcessor::GetLane1PointsGlobal() const {
    return lane1_points_global_;
}

const std::vector<interface::Point2D>& LaneProcessor::GetLane2PointsGlobal() const {
    return lane2_points_global_;
}

const std::vector<interface::Point2D>& LaneProcessor::GetLane3PointsGlobal() const {
    return lane3_points_global_;
}

const std::vector<interface::Point2D>& LaneProcessor::GetLane4PointsGlobal() const {
    return lane4_points_global_;
}
